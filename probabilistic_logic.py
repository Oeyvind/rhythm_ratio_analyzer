...
class Probabilistic_encoder:
    ...
    def get_item_indices(self, item=None):
        ...
        if item and (item not in self.stm.keys()):
            if self.wraparound == 1:
                #print(f'Prob_encoder: {self.name} dead end at key {item}, wrap around ')
                #print(f'key: {item}, allkeys: {self.stm.keys()}')
                return self.wraparound_index_container
            else:
                #print(f'Prob_encoder: {self.name} dead end at key {item}, choose randomly')
                #print(f'key: {item}, allkeys: {self.stm.keys()}')
        ...
        if len(self.stm.keys()) == 0:
            ...
            return [-1.0]
        # if we do not ask for a specific item, choose one randomly
        # same applies if we ask for a nonexisting key
        if (item == None) or (item not in self.stm.keys()):
            #print(f'Prob_encoder: {self.name} previous {previous} not in stm.keys')
            indices = self.empty_index_container
        else:
            indices = self.stm[item] # get an index container of possible next items
        return indices
...
class Probabilistic_logic:
    ...
    def __init__(self, dc, max_size=100, max_order=2, max_voices=10):
        ...
        prob_history = []
        for i in range(self.max_order): prob_history.append(-1)
    ...
    def set_request_mask(self, request_parm, request_code):
        ...
        if request_type == '==':
            if request_parm == 'index':
                ...
                offset = self.max_order
                request = pe.get_item_indices(request_next_item_closest)[offset:self.current_datasize+offset]
...
class Datacontainer_test():
    def __init__(self, max_events, random_population=0): 
        ...
        if random_population == 0:
            self.list_val1 = [1,2,2,1,3,4,2, 3,4,5,-1,-2] 
...
def basic_test():
    ...
    #generate
    pl.set_weights_pname('val2', 0, printit=True)
    pl.set_weights_pname('val1', 2, printit=True)
    temperature = 1 # low (<1.0) is deterministic, high (>1.0) is more random
    ...
    while i < 10:
        ...
        print(f"the next item is  {dc.corpus[next_item_index,dc.pnum_corpus['val1']]} at index {next_item_index}, prob from {dc.corpus[previous,dc.pnum_corpus['val1']]}: {pl.prob}")
        i += 1
    ...
    while i < 10:
        ...
        if (request_item == 'index') and (request_type == '=='):
...
def request_dev():
    ...
    request_parm, request_code, request_weight = request_next_item
    pl.set_request_mask(request_parm, request_code)
    print(f'mask: {pl.request_mask[:pl.current_datasize]}')
    
    request_next_item = ['val1', ['==', [2.6]], 1] # list of one value
    print(f'request mask for {request_next_item}:')
    request_parm, request_code, request_weight = request_next_item
    pl.set_request_mask(request_parm, request_code)
    print(f'mask: {pl.request_mask[:pl.current_datasize]}')
    ...
    request_next_item = ['index', ['gradient', [1]], 1] 
    print(f'request mask for {request_next_item}:')
    request_parm, request_code, request_weight = request_next_item
    pl.set_request_mask(request_parm, request_code)
    print(f'mask: {pl.request_mask[:pl.current_datasize]}')
    
    request_next_item = ['index', ['gradient', [2]], 1] 
    print(f'request mask for {request_next_item}:')
    request_parm, request_code, request_weight = request_next_item
    pl.set_request_mask(request_parm, request_code)
    print(f'mask: {pl.request_mask[:pl.current_datasize]}')
