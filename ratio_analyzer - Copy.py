...
def ratio_scores(ratios, ratios_commondiv, timeseries):
    ...
    for i in range(len(ratios)):
        ...
        indig = 0
        for num in ratios_commondiv[i,:,0]:
            indig += indigestability_n(num)
  ...
  for i in range(2):
    ...
    for j in range(len(p1)):
      ...
      if n <= 32:
        if n%3==0: 
            pulse_3 += coef
            if n%2==0: pulse_2 += (coef*0.5)
        elif n%2==0: pulse_2 += coef
    #print(pulse_2, pulse_3)
...
def analyze(t, rank=1):
    ...
    ratios_commondiv = make_commondiv_ratios(ratios_copy)
    ratios_commondiv_copy = np.copy(ratios_commondiv)
    ratio_deviations, ratio_deviation_abs, ratio_deviation_abs_max, gridsize_deviations, barlow_indigest,benedetti_height, nd_add = ratio_scores(ratios, ratios_commondiv, timedata)
...
if __name__ == '__main__':
    ...
    #for i in range(len(ratios_reduced)):
    #    print(i, '\n', ratios_reduced[i])
