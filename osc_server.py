...
class Osc_server():
    ...
    def receive_eventchord(self, unused_addr, *osc_data):
        ...
        self.corpus[index, self.pnum_corpus['chord_index']] = chord_index+1 # 1-indexed, as zero means no chord
        base_note = self.corpus[index, self.pnum_corpus['notenum']] 
        base_velocity = self.corpus[index, self.pnum_corpus['velocity']]
    ...
    def pl_generate(self, unused_addr, *osc_data):
        ...
        chord_index = self.corpus[next_item_index, self.pnum_corpus['chord_index']]
        if chord_index > 0:
            for event in self.dc.chord_list[int(chord_index-1)]:
                returnmsg = [int(next_item_index), 
                             ...
                             event[2], # deviation (...)
                             float(self.corpus[next_item_index, self.pnum_corpus['duration']]),
                             event[0]+float(self.corpus[next_item_index, self.pnum_corpus['notenum']]),
                             event[0]+float(self.corpus[next_item_index, self.pnum_corpus['notenum_relative']]),
                             event[1]*float(self.corpus[next_item_index, self.pnum_corpus['velocity']])]
